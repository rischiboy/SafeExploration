...
def run_experiment():
    global config
    experiment_cfg = config.pitch_control
...
def experiment(config: dict):
    ...
    env_args = {
        "init_angle": config.init_angle,
        "desired_angle": config.desired_angle,
        "angle_tolerance": config.angle_tolerance,
        "stability_duration": config.stability_duration,
        "max_steps": config.max_steps,
    }

    env = PitchControlEnv(**env_args)
    ...
    # System
    if SAFE:
        cost = PitchControlCost(max_angle=config.max_angle)
        # dynamical_system = SafePitchControlSystem(env=env, dynamics=dynamics, cost=cost)
        dynamical_system = SafePitchControlSystem(dynamics=dynamics, cost=cost)
    else:
        # dynamical_system = PitchControlSystem(env=env, dynamics=dynamics)
        dynamical_system = PitchControlSystem(dynamics=dynamics)

    # env.set_dynamical_sytem(dynamical_system=PitchControlSystem())
...
def main(cfg: DictConfig):
    ...
    experiment_cfg = cfg.pitch_control
    # exp_wandb = experiment_cfg.wandb
...
def eval_string_args(cfg):
    ...
    cfg.pitch_control.model.dynamics = eval(
        "DynamicsType." + cfg.pitch_control.model.dynamics
    )
    cfg.pitch_control.model.optimizer = eval(
        "OptimizerType." + cfg.pitch_control.model.optimizer
    )
    cfg.pitch_control.model.agent = eval("AgentType." + cfg.pitch_control.model.agent)

    if cfg.pitch_control.model.dynamics == DynamicsType.TRUE:
        return cfg

    else:
        cfg.pitch_control.model.sampling_mode = eval(
            "SamplingMode." + cfg.pitch_control.model.sampling_mode
        )
        # cfg.pitch_control.model.output_stds = eval(cfg.pitch_control.model.output_stds)
        # cfg.pitch_control.model.beta = eval(cfg.pitch_control.model.beta)
        # cfg.pitch_control.model.features = eval(cfg.pitch_control.model.features)
...
def assert_proper_args(cfg):
    ...
    assert (
        cfg.pitch_control is not None
    ), "Please provide a pitch control configuration."
    assert (
        cfg.pitch_control.model is not None
    ), "Please provide a model configuration for the pitch control."

    model_cfg = cfg.pitch_control.model
    ...
    if optimizer == OptimizerType.MinMax:
        assert (
            cfg.pitch_control.minmax is not None
        ), "Please provide a minmax configuration."

    elif optimizer == OptimizerType.CEM:
        assert cfg.pitch_control.cem is not None, "Please provide a cem configuration."
    elif optimizer == OptimizerType.ICEM:
        assert (
            cfg.pitch_control.icem is not None
        ), "Please provide a icem configuration."
        assert (
            cfg.pitch_control.icem.exponent is not None
        ), "Please provide an exponent."
    else:
        raise NotImplementedError
    ...
    if agent == AgentType.CEM:
        ...
        assert cfg.pitch_control.constraint is not None, "Please provide a constraint."
