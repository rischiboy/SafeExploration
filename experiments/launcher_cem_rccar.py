...
job_config_file = f"{objective.name.lower()}_{dynamics_model.name.lower()}_rccar"

launch_cmd_args = {
    "rccar": job_config_file,  # Experiment configuration file
}
...
def main(cfg: JobConfig):
    import cem_rccar_test as experiment
    ...
    for _ in range(params.num_hparam_samples):
        if params.launch_mode == "euler":
            ...
            logs_dir = "experiments/results/rccar/"

        exp_base_path = os.path.join(RESULT_DIR, "rccar")
        ...
        for j in range(params.num_seeds_per_hparam):
            ...
            cmd = generate_base_command(
                experiment,
                flags=dict(**flags, **{"rccar.train.seed": seed}),
                is_hydra=True,
            )
