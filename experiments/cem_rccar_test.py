...
def experiment(config: dict):
    ...
    config.init_pose = jnp.array(config.init_pose)
    config.goal = jnp.array(config.goal)
    ...
    env_args = {
        ...
        "goal": config.goal,
        "dt": config.dt,
        "ctrl_cost_weight": config.action_cost,
        "encode_angle": config.encode_angle,
        "max_steps": config.max_steps,
        "seed": config.seed,
    }

    env = RCCarSimEnv(**env_args)
    ...
    if config.dynamics == DynamicsType.TRUE:
        ...
        dynamics = RCCarDynamics(dt=config.dt, encode_angle=config.encode_angle)

    elif config.dynamics == DynamicsType.BNN:
        config.bnn_type = eval(config.bnn_type)
        config.output_stds = jnp.ones(output_dim) * config.output_stds
        config.beta = jnp.ones(output_dim) * config.beta
    ...
    true_dynamics = RCCarDynamics(dt=config.dt, encode_angle=config.encode_angle)
    ...
    if SAFE:
        ...
        dynamical_system = SafeRCCarSystem(dynamics=dynamics, reward=reward, cost=cost)
        val_dynamical_system = SafeRCCarSystem(
            dynamics=true_dynamics, reward=reward, cost=cost
        )

    else:
        dynamical_system = RCCarSystem(dynamics=dynamics, reward=reward)
        val_dynamical_system = RCCarSystem(dynamics=true_dynamics, reward=reward)

    env.set_dynamical_system(val_dynamical_system)

    """ Planner """
    assert (
        dynamical_system is not None
    ), "Could not initialize the rccar dynamical system."
...
def eval_string_args(cfg):
    ...
    cfg.rccar.model.dynamics = eval("DynamicsType." + cfg.rccar.model.dynamics)
    cfg.rccar.model.optimizer = eval("OptimizerType." + cfg.rccar.model.optimizer)
    cfg.rccar.model.agent = eval("AgentType." + cfg.rccar.model.agent)

    if type(cfg.rccar.env.dt) is str:
        cfg.rccar.env.dt = eval(cfg.rccar.env.dt)

    if cfg.rccar.model.dynamics == DynamicsType.TRUE:
        return cfg

    else:
        cfg.rccar.model.sampling_mode = eval(
            "SamplingMode." + cfg.rccar.model.sampling_mode
        )
        # cfg.rccar.model.output_stds = eval(cfg.rccar.model.output_stds)
        # cfg.rccar.model.beta = eval(cfg.rccar.model.beta)
        # cfg.rccar.model.features = eval(cfg.rccar.model.features)

    if SAFE:
        cfg.rccar.constraint.x_bounds = jnp.array(cfg.rccar.constraint.x_bounds)
        cfg.rccar.constraint.y_bounds = jnp.array(cfg.rccar.constraint.y_bounds)
...
def assert_proper_args(cfg):
    ...
    assert cfg.rccar is not None, "Please provide a rccar configuration."
    assert (
        cfg.rccar.model is not None
    ), "Please provide a model configuration for the rccar."

    model_cfg = cfg.rccar.model
    ...
    if optimizer == OptimizerType.MinMax:
        assert cfg.rccar.minmax is not None, "Please provide a minmax configuration."

    elif optimizer == OptimizerType.CEM:
        assert cfg.rccar.cem is not None, "Please provide a cem configuration."
    elif optimizer == OptimizerType.ICEM:
        assert cfg.rccar.icem is not None, "Please provide a icem configuration."
        assert cfg.rccar.icem.exponent is not None, "Please provide an exponent."
    else:
        raise NotImplementedError
    ...
    if agent == AgentType.CEM:
        ...
        assert cfg.rccar.constraint is not None, "Please provide a constraint."
