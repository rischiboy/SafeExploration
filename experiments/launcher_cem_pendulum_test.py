...
from type_utils import DynamicsType, Objective
from util import (
...
config_name = "pendulum_job.yaml"
cs = ConfigStore.instance()
cs.store(name=config_name, node=JobConfig)

objective = Objective.OptMinMax  # ["UNSAFE", "SAFE", "MinMax", "OptMinMax"]
dynamics_model = DynamicsType.BNN  # ["TRUE", "BNN", "GP"]

job_out_dir = f"{objective.name}/{dynamics_model.name}"
job_config_file = f"{objective.name.lower()}_{dynamics_model.name.lower()}_pendulum"

launch_cmd_args = {
    "pendulum": job_config_file,  # Experiment configuration file
}
...
def main(cfg: JobConfig):
    import cem_pendulum_test as experiment
    ...
    for _ in range(params.num_hparam_samples):
        if params.launch_mode == "euler":
            ...
            logs_dir = "experiments/results/pendulum/"
        ...
        for j in range(params.num_seeds_per_hparam):
            ...
            cmd = generate_base_command(
                experiment,
                flags=dict(**flags, **{"pendulum.train.seed": seed}),
                is_hydra=True,
            )
